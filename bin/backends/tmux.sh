...
# Every one of those reads is aimed at the exact resolved target below, never at
# the caller's raw string, so tmux's silent active-window fallback cannot let
# another pane's processes answer for this one.
fm_backend_tmux_agent_state() {  # <target> [recorded-window-id]
  ...
  foreground=$(fm_backend_tmux_foreground_comms "$command_target")
...
EOF
  ...
  argv0s=$(fm_backend_tmux_foreground_argv0s "$command_target")
  while IFS= read -r name; do
    [ -n "$name" ] || continue
