...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.status" "$sub_state/$child_id.turn-ended" \
      ...
      "$sub_state/$child_id.decision-answers" \
...
# This home guards a REGISTERED commitment whether or not outward relay
# authorization is still present. Losing the token cannot retire a promise that
# was already made: the registration stays visible and cleanup stays refused
# until it is delivered or explicitly waived
# (tests/fm-public-followup.test.sh, "cleanup must remain guarded after relay
# token loss"). A route that legitimately puts the promise out of scope - a
# parent on another machine - clears PUBLIC_FOLLOWUP_STATE above instead, so
# that case still cleans up.
if [ "$FORCE" != "--force" ] \
...
quiesce_treehouse_task_endpoint() {
  local attempt=0 expected_label="fm-$ID"
  ...
  if [ "$HERDR_PRESENTATION_RETIRE_CANDIDATE" = 1 ]; then
    ...
    # restore. Swallowing them left a wrong active workspace with no
    # operator-visible signal at all. The close stays non-fatal: the endpoint
    # quiesce proof below is what decides whether the worktree may be returned.
    ...
    teardown_herdr_session_lock_held "$TEARDOWN_HERDR_SESSION" || {
      echo "REFUSED: herdr session presentation lock unavailable; preserving task records and worktree." >&2
      return 1
    }
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  "$STATE/$ID.lifecycle" "$STATE/$ID.surfaced" \
  "$STATE/$ID.pi-ext.ts" "$STATE/$ID.grok-turnend-token" \
  "$STATE/$ID.kimi-turnend-token" "$STATE/$ID.decision-answers" \
