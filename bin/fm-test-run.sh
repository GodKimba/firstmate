...
family_for_basename() {
  case "$1" in
    ...
    fm-ci-workflow.test.sh|fm-composer-ghost.test.sh|fm-composer-lib.test.sh|\
    fm-crew-state.test.sh|fm-decision-answer-authority.test.sh|fm-decision-cutover-migrate.test.sh|\
    fm-decision-hold-lifecycle.test.sh|\
    fm-documentation-audiences.test.sh|fm-ensure-agents-md.test.sh|fm-grok-harness.test.sh|\
    fm-kimi-harness.test.sh|fm-herdr-lab.test.sh|fm-lint.test.sh|\
    fm-operational-input.test.sh|fm-pi-primary-types.test.sh|fm-pi-tool-result-images.test.sh|\
    ...
    fm-grok-stop-live-e2e.test.sh|fm-opencode-primary-live-e2e.test.sh|\
    fm-pi-primary-live-e2e.test.sh|fm-send-claude-vim-recovery-live-e2e.test.sh|\
    fm-send-secondmate-marker-herdr-e2e.test.sh)
      printf '%s\n' live-harness-optin
      ;;
    fm-backend-herdr.test.sh|fm-backend-tmux-smoke.test.sh|fm-backend.test.sh|\
    fm-herdr-session-cleanup.test.sh|fm-send-claude-vim-recovery.test.sh|\
    fm-send-strict.test.sh|fm-spawn-batch.test.sh|\
    fm-spawn-dispatch-profile.test.sh|fm-spawn-scout-branchless.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-daemon*|bin/fm-turnend-guard*|bin/fm-guard.sh)
      printf '%s\n' watcher-wake-lock
      ;;
    # The keyed-decision fold has consumers well beyond the watcher: the crew-state
    # deriver and brief scaffolds, the fleet snapshot, and the away-mode return gate.
    bin/fm-classify-lib.sh)
      ...
      printf '%s\n' backend-dispatch
      ;;
    # The away-mode stand-down contract straddles both sides: the lifecycle
    # scripts here own the flag and the shutdown wait, while the arm layer, the
    # adapters and the owner-tagged watcher lock that read them are covered by
    # the watcher family.
    bin/fm-afk*)
      printf '%s\n' afk
      printf '%s\n' real-herdr-gated
    ...
    bin/fm-decision-hold.sh|bin/fm-decision-cutover-migrate.sh|\
    bin/fm-supervision*|bin/fm-transition-lib.sh|\
