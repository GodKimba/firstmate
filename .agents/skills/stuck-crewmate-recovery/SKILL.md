...
Use `treehouse status` for treehouse-backed tmux, herdr, zellij, or cmux tasks.
For an Orca task, load `firstmate-orca` and follow its recorded successful-task or failed-spawn recovery shape; a valid recovery record may intentionally lack a terminal or absolute worktree path.
...
3. If the crewmate is confused or looping, interrupt with the adapter's verified procedure, then redirect with one corrective line.
   For Claude, use the `harness-adapters` skill's Claude Vim interruption recovery command; a raw Escape delivery is not interruption proof, and pending recovery may require Enter-only continuation instead of retyping.
   For a non-Claude single-Escape adapter, use `FM_HOME=<this-firstmate-home> bin/fm-send.sh <window> --key Escape`.
