...
The nightly and manually dispatched complete-suite Herdr job uses the pinned installers in `bin/fm-install-herdr.sh` and `bin/fm-install-treehouse.sh`.
...
The recorded endpoint must then be authoritatively absent before Treehouse return can proceed.
An unavailable lock, still-present pane, or unreadable endpoint inventory refuses teardown and preserves the task record, worktree, and presentation journal.
A journal that cannot be correlated to the confirmed exact close stays quarantined for manual inspection.
...
The watcher maps the pane back to the task and skips secondmate endpoints plus declared `paused:` waits that authoritative current state still confirms on a live endpoint.
The shared classifier gives open keyed decisions and blockers precedence over a later pause, while dead, unreadable, and non-paused human-blocked endpoints still surface immediately.
...
Inside Herdr, its before/after tripwire identifies the live fleet session from complete socket and pane identity, so a stopped `default` session is not required.
It refuses incomplete ambient identity instead of falling back, while callers outside Herdr retain compatibility with one running `default` session.
The tripwire requires the recorded session snapshot, including its socket path, to remain byte-identical and revalidates the recorded pane for an ambient Herdr caller.
...
Real Herdr tests use the named lab helper and its exact recorded fleet-session tripwire.
