...
| `fm-pool-quota.sh`       | Read the local subscription pool's accounts through `quota-axi`, project the quota view, and regenerate its private local panel |
| `fm-pool-preflight.sh`   | Judge whether a long validation may start on the measured headroom; an admission check that binds no account |
...
| `fm-classify-lib.sh`     | Shared status fold and supervision precedence across decisions, blockers, pauses, current state, and endpoint liveness |
