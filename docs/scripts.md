...
| `fm-decision-cutover-migrate.sh` | Report, and with `--apply` upgrade, one home's eligible legacy decision histories onto correlated answer authority; refuses by name anything it cannot prove safe |
...
| `fm-primary-scope-lib.sh` | Shared launch-identity and checkout-shape primary-home predicate for tracked hooks   |
...
| `fm-startup-memory-budget-lib.sh` | Validate, materialize, and measure the primary-authoritative startup-memory budget |
| `fm-startup-memory-budget.sh` | Read the effective startup-memory budget or report its three-file estimate            |
...
| `fm-classify-lib.sh`     | Shared wake-classification vocabulary, durable keyed-decision folds and scans, and supervision precedence across decisions, blockers, pauses, current state, and endpoint liveness |
| `fm-send.sh`             | Send one verified literal line, supported key, or correlated answer to an open decision through the target's recorded backend |
...
| `fm-pr-check.sh`         | Record validated `pr=`, `pr_head=`, and optional required ancestry, then atomically arm a static merge poll |
...
| `fm-promote.sh`          | Promote a scout task in place to a protected ship task with an explicit delivery mode and an acquired branch |
