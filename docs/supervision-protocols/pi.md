...
6. After an actionable arm cycle settles from `close` or the bounded post-exit drain, the extension rechecks session-lock ownership and verifies one successor before it delivers the follow-up wake; its bounded fallback is defined in `docs/watcher-continuity.md`.
...
8. An unexpected arm cycle settles through `close` or the same bounded post-exit drain and enters bounded exponential retry; a `close` that arrives after process-exit settlement only finalizes streams and cannot settle the cycle twice.
   An exhausted retry or lost session lock is surfaced as a watcher failure instead of disappearing.
9. Missing, failed, or unhealthy cycle only: if a later notification explicitly reports one of those repair conditions, drain queued wakes, inspect the failure text, call `fm_watch_arm_pi`, and restart the selected Pi-family executable with both extensions loaded if needed.
   A redundant call while the extension owns a live arm child or scheduled retry is an ownership-based `watcher: unchanged` no-op, not an independent health claim.
