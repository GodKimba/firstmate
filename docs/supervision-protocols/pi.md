...
7. After an actionable arm settles from `close` or the bounded post-exit drain, the extension rechecks session-lock ownership and verifies one successor before it delivers the follow-up wake; a later `close` only finalizes streams and cannot settle the cycle twice.
   Its bounded fallback is defined in `docs/watcher-continuity.md`.
...
9. An unexpected arm settlement enters bounded exponential retry, and an exhausted retry or lost session lock is surfaced as a watcher failure instead of disappearing.
   While away mode is active, the extension starts no arm, cancels pending retries, treats an arm-layer `stood-down` result as a terminal non-failure, and delivers no duplicate wake because the away supervisor owns the single watcher.
