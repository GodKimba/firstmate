...
| Pi / pi-signed | Run | `.pi/extensions/fm-primary-turnend-guard.ts` maps `session_start` reasons `startup`, `new`, `resume`, and `fork` onto wrapper sources, handles `session_compact` as the compaction equivalent, and injects the output with `pi.sendMessage`. It applies the shared launch identity itself, so a crewmate never spawns the wrapper at all. | The custom message reaches model context without racing an initial positional prompt. Pi's `reload` reason is deliberately unmapped, as it always was. |
...
The same file proves wrapper silence for a stamped crewmate in a primary-shaped checkout, with an inherited coordinator home, and against a valid secondmate marker, and proves a cleared identity still opens the session.
It drives the real Pi `session_start` handler to prove a crewmate never spawns a wrapper while a coordinator spawns one every time.
