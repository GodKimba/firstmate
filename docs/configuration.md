...
It emits `SECONDMATE_SYNC:` only when a home was skipped for an actionable sync reason, inheritance failed, a divergent shared captain-preference copy was quarantined, or a live secondmate is quarantined from dispatch and decision answers until its compatible code reload is acknowledged.
