...
The retry never allocates a replacement copy, and diagnostics preserve every Git failure, including the initiating detach error and any retry error.
...
After validation, a single-object rule or default is the authoritative route; quota observations can report or defer it but cannot substitute another profile.
Profile `model` and `effort` fields and rule `why` are optional.
An omitted model or effort means the selected harness uses its own default for that axis.
Every profile array is an implicit quota-aware choice resolved through `quota-array-dispatch`, limited to the profiles and harness families the array actually offers.
A profile routed through the local CLIProxyAPI account pool is one candidate whose aggregate pool health is read through `bin/fm-pool-preflight.sh --pool`; the proxy, not Firstmate, selects its account, and unrelated direct-login state does not gate it.
...
An installed tool below its floor reports the normal `MISSING` update command and waits for explicit approval.
[`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh) separately re-checks gh-axi's own `api --jq` capability at the merge boundary, so a mutation never trusts a session-start reading.
The per-backend delta is required only for the backend resolved from `FM_BACKEND`, then `config/backend`, then runtime auto-detection, then default `tmux`, so a home is never told to install a tool an inactive backend or feature would need.
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, or `cmux`), `jq` for the JSON-emitting adapters (`herdr`, `zellij`, `cmux`), and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`).
...
Registration is what creates this home's private transport under `state/public-followup/` (mode 0700): `registry/<obligation-id>/<relation-id>` for each bounded public-safe work binding, `events/` for typed terminal results awaiting reconciliation, `consumed/` for the accepted-event ledger, `rejected/` for refusals kept with a one-line reason, and `surfaced` for the poll's last-surfaced signature.
...
Work routed elsewhere reports a typed terminal result with `bin/fm-public-followup-emit.sh` and never looks for the thread; that emitter requires the exact registered obligation and relation.
...
FM_BACKEND_HERDR_SUBMIT_POLLS=6  # herdr-only: agent-state samples spread across each Enter attempt's budget when confirming a submit from an idle baseline (docs/herdr-backend.md "Current transport behavior")
...
FM_CREW_STATE_GH_TIMEOUT=20   # seconds allowed for the one GitHub check-run probe behind a green-ready verdict on no-mistakes work
...
FM_OPENCODE_ARM_READY_TIMEOUT_MS=12000   # milliseconds the OpenCode primary watcher plugin waits for an arm attempt to report started, healthy, wake, stood-down, or failure; default 35000 on Windows to stay above the MSYS confirm budget
FM_PI_ARM_READY_TIMEOUT_MS=12000   # milliseconds the Pi watcher extension waits for a successor arm to report started, attached, or stood-down; default 35000 on Windows to stay above the MSYS confirm budget
FM_PI_ARM_EXIT_DRAIN_MS=50   # milliseconds the Pi watcher extension drains already-buffered stdout/stderr after arm-process exit
...
FM_CAPTAIN_RE='(done|needs-decision|blocked|failed)([[:space:]]|:)|PR ready|checks green|ready in branch|merged'   # captain-relevant status regex; keyed and unkeyed terminal verbs match, while nonterminal progress verbs remain excluded even when their prose matches
FM_CLASSIFY_PAUSED_VERB=paused     # leading status verb for a declared external wait; excluded from FM_CAPTAIN_RE and distinct from blocked
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; other stale panes surface immediately unless shared precedence confirms a valid long-cadence wait
FM_BUSY_TURN_MAX_SECS=3600         # maximum age since the later of a busy pane's trusted current semantic transition and latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; recordless busy verdicts remain exempt until a trusted transition establishes a duration anchor; inspection-only, never an automatic interrupt or restart
FM_PAUSE_RESURFACE_SECS=3600       # seconds before a current declared pause on a live endpoint, or a verified dead-agent captain hold, re-surfaces for a recheck
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added
FM_WATCH_TRIAGE_LOG_MAX_BYTES=262144   # size cap for the watcher's absorbed-wake debug log
FM_FLEET_SYNC_BOOTSTRAP_TIMEOUT=     # optional seconds allowed for bootstrap's best-effort clone refresh; unset/blank defaults to max(20, 5 + 3 * origin-backed-project-count)
FM_FLEET_PRUNE=1        # set to 0 to skip pruning local branches whose upstream is gone
FM_WORKTREE_SETTLE_POLLS=60              # positive integer bound for fm-spawn.sh worktree discovery after treehouse get; unset, blank, zero, or invalid uses 60
FM_WORKTREE_SETTLE_INTERVAL=1            # positive seconds between those polls; unset, blank, zero, or invalid uses 1
...
FM_ENDPOINT_QUIESCE_RETRIES=20            # follow-up inventory checks while fm-teardown.sh confirms the recorded endpoint is gone
FM_ENDPOINT_QUIESCE_WAIT_SECS=0.1         # seconds between those endpoint-absence checks
...
# away-mode lifecycle and sub-supervisor; presence-gated via /afk
FM_AFK_STOP_TIMEOUT=45            # seconds fm-afk-launch waits before one exact-identity shutdown reconciliation; docs/watcher-continuity.md owns the rationale and ambiguity contract
...
`FM_ENDPOINT_QUIESCE_RETRIES` accepts a nonnegative integer and falls back to 20 when unset, blank, or invalid.
`FM_ENDPOINT_QUIESCE_WAIT_SECS` accepts nonnegative whole or fractional seconds and falls back to 0.1 when unset, blank, or invalid.
Teardown retains task records and the worktree when the recorded endpoint remains present or cannot be authoritatively queried after that bounded confirmation window.
