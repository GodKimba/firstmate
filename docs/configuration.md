...
FM_WORKTREE_SETTLE_POLLS=60              # polls fm-spawn.sh waits for the backend to report the exact isolated worktree root after treehouse get; a nested directory a slow provider is still provisioning under is ignored, never normalized upward
FM_WORKTREE_SETTLE_INTERVAL=1            # seconds between those polls; both accept a positive value and fall back to the default when unset, blank, or invalid
