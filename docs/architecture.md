...
Actionable wakes include captain-relevant status signals, newly observed open keyed decision or blocker occurrences retained by the durable status fold, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes without a valid long-cadence wait whose crew is not provably working, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, valid declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A busy pane with a trusted current semantic transition is otherwise exempt from staleness only until the later of that transition and its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, using its `state/<id>.meta` spawn record before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
A recordless native or rendered-tail busy verdict remains exempt until a trusted semantic transition establishes a valid duration anchor.
...
Dead or unreadable endpoints and pause declarations that no longer match current state surface immediately instead of inheriting the pause cadence.
A verified `captain-held` transfer is the separate exited-agent exception and may use that cadence when the backend confidently reports the endpoint dead.
The ordinary idle-secondmate exemption remains, but paused secondmates and unreadable captures are checked so endpoint loss still surfaces once.
...
A valid declared external wait trades that silence for one bounded recheck per pause window, so a forgotten pause cannot remain invisible indefinitely.
...
Only when no matching run exists does it consult semantic busy state; exact busy reports working, and exact idle permits the ordinary fallback to a status-log event whose verb maps to a recognized run-state.
An unknown semantic verdict permits only a corroborated green-ready report or a durable `needs-decision`, `blocked`, or `paused` event to retain status-log authority, while every other progress or completion event remains unknown; a dead or unreadable endpoint stays unknown before any log fallback.
Decision-only events such as `resolved` never become current state or leak their prose into the current-state detail.
In that status-log fallback, a declared external wait reports the distinct `paused` state with its reason.
The semantic branch reports working only on an exact busy verdict and names the source that produced it; an unknown verdict never becomes working or a silent idle, and exposes only the narrow authoritative exceptions above.
...
[`watcher-continuity.md`](watcher-continuity.md) owns Pi and OpenCode session-lock verification, singleton-successor ordering, process-settlement boundaries, and bounded restoration retries.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, the keyed decision and blocker fold, declared-external-wait vocabulary, and supervision precedence.
...
The always-on watcher uses that shared precedence for candidate pauses, first-sighting stale panes, bounded pause rechecks, and native push blockers, while the daemon applies it to the same paths before maintaining distinct wedge and pause cadences.
...
The daemon immediately escalates invalid pause candidates and endpoint loss, while a valid declared pause that remains idle receives one bounded recheck as a batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
...
Herdr confirms a busy send by requiring the composer to hold the message before Enter and to drain after it, rather than inferring acceptance from a pane that was already busy; `docs/herdr-backend.md` owns that boundary and the opencode shape still open there.
...
Orca is experimental and selected only explicitly: Orca owns both worktree and terminal lifecycle and removes worktrees through `orca worktree rm` only after the usual firstmate teardown checks pass.
Successful-task and failed-spawn recovery metadata follow the variants owned by [`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend).
...
The teardown and generic-helper headers own the exact checks, sequencing, and retry mechanics.
...
That keeps spawn launch compatible across the verified worker harnesses while preserving the requested profile for later audit.
...
A home that never registered a commitment executes none of it, while an existing registration remains locally visible and guarded if outward relay authorization disappears.
