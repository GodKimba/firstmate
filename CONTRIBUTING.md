...
CI runs a bounded routine proof on pull requests and keeps broad regression across portable parallel shards, the portable serial lane, the Herdr lane, lint, invariants, the coverage guard, and macOS snapshot compatibility in nightly and manual complete-suite runs.
[`docs/fm-test-portable-shards.md`](docs/fm-test-portable-shards.md) owns the routing rationale, timing evidence, concurrency policy, and artifact posture, while [`.github/workflows/ci.yml`](.github/workflows/ci.yml) owns executable wiring.
