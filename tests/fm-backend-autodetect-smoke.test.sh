...
# bin/fm-herdr-lab.sh, which appends the named session flag and verifies its
# recorded fleet-session tripwire after teardown. Never replace the helper with
# an ambient HERDR_SESSION-only command.
set -u
