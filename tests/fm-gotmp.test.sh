...
make_fake_root() {
  ...
  # fm-classify-lib.sh comes with it: wake-lib sources the classifier for the
  # decision wake kind and the stream-marker skip, so a fake root that offers
  # one without the other cannot source either.
