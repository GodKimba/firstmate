...
test_ci_installs_and_logs_the_pinned_version() {
  ...
  [ "$(grep -Fc "bin/fm-install-shellcheck.sh \"\$RUNNER_TEMP/bin\"" "$CI")" -eq 6 ] || fail "both routine jobs plus lint and all three complete portable jobs must use the shared ShellCheck installer"
