...
The verified harnesses are `claude`, `claude-pool`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, and `kimi`; never dispatch on an unverified adapter.
`claude-pool` runs the Claude CLI on the local CLIProxyAPI pool for crewmate and secondmate launches only, requires an explicit Claude model, and never falls back to the native Claude login.
...
Configured harness-family intent also remains authoritative: pace selection may compare only the candidates in the matched array and must not move an Anthropic-family task onto a GPT-family route, or the reverse, by analogy.
...
The promoted worker must inventory scratch state, reset the prepared ship branch to a clean default-branch base, carry over only intended fix changes, and follow the project's selected delivery path while leaving scratch commits and debug edits behind and turning a reproduced bug into the regression test.
